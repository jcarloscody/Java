...
public class DepartamentFormController implements Initializable {
	...
	private Departament getFormData() {
		...
		ValidationException exception = new ValidationException("Validation error");//instaciou a excecaoda classe criada
		
		obj.setId(Utils.tryParseToInt(txtId.getText()));
		
		if (txtName.getText() == null || txtName.getText().trim().equals("")) {
			exception.addError("name", "Field can't be empty");//add erro no map da classe
		}
		obj.setName(txtName.getText());
		
		if (exception.getErrors().size() > 0) {
			throw exception;//lancando a excecao
		}
	...
	private void setErrorMessages(Map<String, String> errors) {
		...
		if (fields.contains("name")) {//se tem a chave name 
